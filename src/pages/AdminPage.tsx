    ...
    <div>
      ...
      {tab === 'dictionary' && (
        <div className="space-y-6">
          <SimpleCrudTable title="أقسام القاموس" items={store.dictSections} search={search} setSearch={setSearch}
            ...
            filterFn={(item) => !search || item.nameAr?.includes(search)}
          />
          <SimpleCrudTable title="مصطلحات القاموس" items={store.dictEntries} search={search} setSearch={setSearch}
      ...
      {tab === 'logs' && (() => {
        const logTypes = ['الكل', 'إنشاء', 'تعديل', 'حذف', 'حظر', 'تصدير', 'استيراد', 'أرشفة', 'تسجيل دخول', 'كلمة مرور'];
        const [logFilter, setLogFilter] = [logTypeFilter, setLogTypeFilter];
        const filtered = store.adminLogs.filter(l => {
          const matchSearch = !search || l.action.includes(search) || l.details.includes(search);
          const matchType = !logFilter || logFilter === 'الكل' || l.action.includes(logFilter);
          ...
          <div className="p-4 border-b border-surface-100">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-bold text-surface-900 flex items-center gap-2">
                ...
                سجلات الإدارة
                <span className="text-xs bg-surface-100 text-surface-500 px-2 py-0.5 rounded-full font-normal">{filtered.length} / {store.adminLogs.length}</span>
              </h2>
            ...
            <input className="w-full border border-surface-200 rounded-lg px-3 py-2 text-sm mb-2" placeholder="ابحث بالاسم أو التفاصيل..." value={search} onChange={e => setSearch(e.target.value)} />
            <div className="flex gap-1.5 flex-wrap">
              {logTypes.map(type => (
                <button key={type}
                  className={cn('px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all border',
                    (logFilter === type || (!logFilter && type === 'الكل'))
                      ? 'bg-primary-500 text-white border-primary-500'
                      : 'bg-white text-surface-500 border-surface-200 hover:border-primary-300'
                  )}
                  onClick={() => setLogFilter(type === 'الكل' ? '' : type)}>
                  {type}
                </button>
              ))}
            </div>
...
// ─── SimpleCrudTable (for Dictionary - no trash/archive) ─────────────────────
// eslint-disable-next-line @typescript-eslint/no-explicit-any
