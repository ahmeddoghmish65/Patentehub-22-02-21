...
export function ProfilePage({ onNavigate }: ProfilePageProps) {
  ...
  const handleSaveEdit = async () => {
    ...
    if (editForm.phone && (rawPhone.length < 7 || rawPhone.length > 15)) {
      setPhoneError('رقم الهاتف غير صالح (7-15 رقم)');
      return;
    }
    setPhoneError('');
    ...
    if (editForm.username && editForm.username !== user.username) {
      ...
      const allUsers = await db2.getAll('users');
      const taken = allUsers.some(u => u.id !== user.id && (u.username || '').toLowerCase() === editForm.username.toLowerCase());
      if (taken) { setSaveMsg('❌ اسم المستخدم مأخوذ، اختر اسماً آخر'); return; }
      if (!/^[a-zA-Z0-9_.]{3,20}$/.test(editForm.username)) { setSaveMsg('❌ اسم المستخدم يجب أن يكون 3-20 حرف (حروف إنجليزية، أرقام، _ أو .)'); return; }
    }
